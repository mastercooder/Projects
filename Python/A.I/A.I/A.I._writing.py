...
def wishMe():
    ...
    # ------------------------------------DEVELOPER-----------------------------------
    if "the developer" in query.lower() or "who is the daveloper" in query.lower() or "is developer" in query.lower():
        print()
        print('''
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def functions(user_input):
    ...
    elif "calculate" in query.lower() or "calculator" in query.lower():
        while True:
            try:
...
def writin_letter():
    ...
    letter_content = generate_writing()
    letter = f"""Sender: {sender_name}
...
''')
    print(f"{timenow} Sir Please let me know If I can help you")
    print()
...
#                                                                                                               --------------
        ...
        # -------------------------------------------------------------------------------------------------YOUTUBE-----------------------------------------------------------------------------------------------------------
            ...
            #   ------------------------------------------------------------------------------HINDI SONGS-----------------------------------------------------------------------------
                ...
                ["ride it" ,"https://youtu.be/ZGKkOEcnocU"],
                ["o re piya" ,"https://youtu.be/hiZrPIZPuCM"],
                ["janam janam" ,"https://youtu.be/CDPzYTccGHs"],
                ["kaise hua" ,"https://youtu.be/96Z8Mp1DEyo"],
                ["tum se hi" ,"https://youtu.be/V-NIDNBw_8E"],
                ["" ,""],
            #   -----------------------------------------------------------------------------ENGLISH SONGS----------------------------------------------------------------------------
    ...
    #                                                                                                                -----
        ...
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if "show sites" in query.lower():
                print("Here is your sitelist sir...")
                print()
            ...
            if "show private sites" in query.lower():
                password = input("Enter password to see the sites: ")
                if password=="1234":
                    print("Here is your playlist sir...")
                    print()
                    for index, item in enumerate(secures, start=1):
                        key = item[0]
                        key = key.capitalize()
                        print(f"{index}: {key}")
                    print()
            ...
            # ----------------------------------------------------------------------APPLICATION------------------------------------------------------
            for app in apps:
                if f"open {app[0]}" in query.lower() or f"launch {app[0]}" in query.lower():
            ...
            if "show apps" in query.lower() or "show applist" in query.lower():
                print("Here is your applist sir...")
                print()
        ...
        # ------------------------------------------------------------------------Songs--------------------------------------------------------------------------
            ...
            if "pause the song" in query.lower() or "stop the song" in query.lower() or "play the song" in query.lower():
                print("Pausing the song sir...")
        ...
        # ------------------------------------------------------------------------Google--------------------------------------------------------------------------
            if "search in google" in query.lower() or "search on google" in query.lower() or "in google" in query.lower() or "on google" in query.lower():
                ...
                print(f"Searching in google {search_query}")
                webbrowser.open(search_url) 

            if "search in youtube" in query.lower() or "search on youtube" in query.lower() or "search youtube" in query.lower() or "play in youtube" in query.lower() or "play on youtube" in query.lower() or "play youtube" in query.lower():
                search_query = query.lower().replace("search in youtube", "")
                encoded_query = urllib.parse.quote(search_query)
                youtube_url = "https://www.youtube.com/results?search_query="
                search_url = youtube_url + encoded_query
                print(f"Searching for {search_query} in YouTube...")
                webbrowser.open(search_url)

            if "search in map" in query.lower() or "search in google map" in query.lower() or "locate" in query.lower() or "navigate" in query.lower():
                search_query = query.lower().replace("search in map", "")
                reaplaced_query = urllib.parse.quote(search_query)
                map_url = "https://www.google.com/maps/search/"
                search_url = map_url + reaplaced_query
                print(f"Locating {search_query} on Google Maps")
                webbrowser.open(search_url)
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------
            if "search in wikipedia" in query.lower() or "search on wikipedia" in query.lower() or "refer to wikipedia" in query.lower():
                ...
                print(f"Searching {search_query} in wikipedia")
                webbrowser.open(search_url)
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------
...
''')
                while True:
                    try:
                        user_inputer = input("Enter The number: ")
                        user_input = int(user_inputer)
                        if "exit" in user_inputer.lower():
                            print()
                            break
                        elif user_input<=0:
...
''')
        ...
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            elif "write letter" in query.lower():
                topic = input("Enter the topic for the letter: ")
                try:
                    num_sentences = int(input("Enter the number of sentences you want to print: "))
                    if num_sentences <= 0:
                        print("Please enter a positive number")
    ...
    #                                                                                                                ------    
        ...
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------
            if "the date" in query.lower() or "the day" in query.lower() or "the time" in query.lower():
                if "the time" in query.lower() or "time" in query.lower():
                    ...
                    print(f"Sir the day is {current_day}")
                elif "the date" in query.lower() or "date" in query.lower():
                    current_date = datetime.date.today()
                    print(f"Sir the date is {current_date}")
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------
            if "hello jarvis" in query.lower() or "how are you jarvis" in query.lower() or "what can you do" in query.lower():
                try:
                    if "hello jarvis" in query.lower():
                        print()
                        user_input = input("Yes sir May I help you: ")
                        print()
                        if "yes" in user_input.lower() or "yap" in user_input.lower():
                            print()
                            print("Tell me what can I do!")
                            print()
                        elif "no" in user_input.lower() or "not really" in user_input.lower:
                            print()
                            print("Please Let me know if I can help you with something")
                            print()
                            sys.exit()

                    elif "how are you" in query.lower():
                        print()
                        user_input = input("I am fine sir! How about you: ")
                        print()
                        if "i am fine" in user_input.lower() or "i am good" in user_input.lower():
                            print()
                            print("OK! sir may I help you with something")
                            print()
                            if "yes" in user_input.lower() or "yap" in user_input.lower():
                                print()
                                print("Tell me what can I do!")
                        ...
                        print()
                        print("I can do many thing like - playing music, opening apps, opening sites, maths probum, and lot more")
                        print()
        ...
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------
            if "weather" in query.lower():
                print("Weather Report of Raigarh Chhattisgarh")
